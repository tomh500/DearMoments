...
private:
    ...
    // ===== 玩家状态 =====
    bool was_playing_this_round = false;

    // simdjson parser 放堆上，避免栈溢出
...
public:
    ...
    void Process(const std::string& body, SteamHelper&) {
        simdjson::dom::element doc;
        if (parser->parse(body).get(doc)) return;

        // ===== 基础字段 =====
        std::string phase = GetStr(doc, "/round/phase");
        std::string activity = GetStr(doc, "/player/activity");

        int kills = GetInt(doc, "/player/state/round_kills", 0);
        int mvps = GetInt(doc, "/player/match_stats/mvps", 0);
        ...
        if (phase != last_phase) {
            ...
            if (phase == "live") {
                last_kills = 0;
                mvps_at_round_start = mvps;
                mvp_candidate_kills = 0;
                mvp_pushed_this_round = false;
                was_playing_this_round = (activity == "playing");

                QueueEvent(-13); // round start
            }

            // ===== 回合结束 =====
            if (last_phase == "live" && phase == "over") {
                HandleKills(kills);
                bool real_mvp =
                    (mvps > mvps_at_round_start) ||
                    (!mvp_pushed_this_round && mvp_candidate_kills > 0);

                if (real_mvp) {
                    QueueEvent(-2); // MVP
                    mvp_pushed_this_round = true;
                }
            }
        ...
        if (phase == "live" && activity == "playing") {
            if (kills > last_kills) {
                // 防止跳 kill（0 -> 2）
                for (int k = last_kills + 1; k <= kills; ++k) {
                    QueueEvent(k);
                }

                mvp_candidate_kills += (kills - last_kills);
                last_kills = kills;
            }
        }
        ...
        if (debug_mode) {
            std::cout
                ...
                << " mvps=" << mvps
                << " mvp_candidate=" << mvp_candidate_kills
                << "\n";
        }
    }
...
private:
    // ===== 工具函数 =====
    std::string GetStr(const simdjson::dom::element& d, std::string_view p) {
        simdjson::dom::element e;
        return (d.at_pointer(p).get(e) == simdjson::SUCCESS)
            ? std::string(e.get_string().value())
            : "";
    }

    int GetInt(const simdjson::dom::element& d, std::string_view p, int def) {
        simdjson::dom::element e;
        return (d.at_pointer(p).get(e) == simdjson::SUCCESS)
            ? (int)e.get_int64().value()
            : def;
    }
...
static GSIHandler g_handler;

// =========================
// HTTP 接收入口
// =========================
void ReceiveData(httplib::Server* svr, bool, float, bool) {
    static SteamHelper helper;
    ...
    svr->Post("/", [](const httplib::Request& req, httplib::Response& res) {
        ...
        g_handler.Process(req.body, helper);
        BroadcastGSI(req.body);//广播到kcore
